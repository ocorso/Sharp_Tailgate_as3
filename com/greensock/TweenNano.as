 ...
 */	 
	public class TweenNano {
		...
		protected static var appModelasterList:Dictionary = new Dictionary(false); 
		...
		public function TweenNano(target:Object, duration:Number, vars:Object) {
			...
			var a:Array = appModelasterList[target];
			if (a == null || int(this.vars.overwrite) == 1 || this.vars.overwrite == null) { 
				appModelasterList[target] = [this];
			} else {
				a[a.length] = this;
			}
...
//---- STATIC FUNCTIONS -------------------------------------------------------------------------
		...
		public static function updateAll(e:Event=null):void {
			_frame++;
			_time = getTimer() * 0.001;
			var ml:Dictionary = appModelasterList, a:Array, tgt:Object, i:int, t:Number, tween:TweenNano;
			for (tgt in ml) {
				a = ml[tgt];
				i = a.length;
				while (--i > -1) {
		 ...
		 */
		 public static function killTweensOf(target:Object, complete:Boolean=false):void {
			if (target in appModelasterList) {
				if (complete) {
					var a:Array = appModelasterList[target];
					var i:int = a.length;
					while (--i > -1) {
				...
				delete appModelasterList[target];
			}
		}
