 ...
 */
	public class TransformItem extends EventDispatcher {
		...
		/** @private **/
		protected var appModelinScaleX:Number;
		/** @private **/
		protected var appModelinScaleY:Number;
		/** @private **/
		protected var appModelaxScaleX:Number;
		/** @private **/
		protected var appModelaxScaleY:Number;
		/** @private **/
			...
			if (_lockScale) {
				...
				if (this.hasScaleLimits) {
					...
					if (angleDif == 0 && skewDif < 0.0001 && skewDif > -0.0001) {
						...
						if (sx > appModelaxScaleX) {
							$safe.sx = appModelaxScaleX / originalScaleX;
							if (_constrainScale) {
						...
						} else if (sx < appModelinScaleX) {
							$safe.sx = appModelinScaleX / originalScaleX;
							if (_constrainScale) {
								$safe.sy = sy = $safe.sx / ratio;
								if ($safe.sx * $safe.sy < 0) { //make sure sy is negative if sx is.
									$safe.sy = sy = -sy;
						...
						if (sy > appModelaxScaleY) {
							$safe.sy = appModelaxScaleY / originalScaleY;
							if (_constrainScale) {
						...
						} else if (sy < appModelinScaleY) {
							$safe.sy = appModelinScaleY / originalScaleY;
							if (_constrainScale) {
								$safe.sx = $safe.sy * ratio;
								if ($safe.sx * $safe.sy < 0) { //make sure sx is negative if sy is.
									$safe.sx *= -1;
						...
						sx = MatrixTools.getScaleX(m);
						sy = MatrixTools.getScaleY(m);
						if (sx > appModelaxScaleX || sx < appModelinScaleX || sy > appModelaxScaleY || sy < appModelinScaleY) {
							$safe.sx = $safe.sy = 1;
							return;
						}
					}
...
//---- GETTERS / SETTERS --------------------------------------------------------------------------------
		...
		public function get minScaleX():Number {
			return appModelinScaleX;
		}
		public function set minScaleX($n:Number):void {
			if ($n == 0) {
				$n = _targetObject.getBounds(_targetObject).width || 500;
				appModelinScaleX = 1 / $n; //don't let it scale smaller than 1 pixel.
			} else {
				appModelinScaleX = $n;
			}
			if (_targetObject.scaleX < appModelinScaleX) {
				_targetObject.scaleX = appModelinScaleX;
			}
			if (_selected) {
				dispatchEvent(new TransformEvent(TransformEvent.UPDATE, [this]));
			}
		...
		public function get minScaleY():Number {
			return appModelinScaleY;
		}
		public function set minScaleY($n:Number):void {
			if ($n == 0) {
				$n = _targetObject.getBounds(_targetObject).height || 500;
				appModelinScaleY = 1 / $n; //don't let it scale smaller than 1 pixel.
			} else {
				appModelinScaleY = $n;
			}
			if (_targetObject.scaleY < appModelinScaleY) {
				_targetObject.scaleY = appModelinScaleY;
			}
			if (_selected) {
				dispatchEvent(new TransformEvent(TransformEvent.UPDATE, [this]));
			}
		...
		public function get maxScaleX():Number {
			return appModelaxScaleX;
		}
		public function set maxScaleX($n:Number):void {
			if ($n == 0) {
				$n = _targetObject.getBounds(_targetObject).width || 0.005;
				appModelaxScaleX = 0 - (1 / $n); //don't let it scale smaller than 1 pixel.
			} else {
				appModelaxScaleX = $n;
			}
			if (_targetObject.scaleX > appModelaxScaleX) {
				_targetObject.scaleX = appModelaxScaleX;
			}
			if (_selected) {
				dispatchEvent(new TransformEvent(TransformEvent.UPDATE, [this]));
			}
		...
		public function get maxScaleY():Number {
			return appModelaxScaleY;
		}
		public function set maxScaleY($n:Number):void {
			if ($n == 0) {
				$n = _targetObject.getBounds(_targetObject).height || 0.005;
				appModelaxScaleY = 0 - (1 / $n); //don't let it scale smaller than 1 pixel.
			} else {
				appModelaxScaleY = $n;
			}
			if (_targetObject.scaleY > appModelaxScaleY) {
				_targetObject.scaleY = appModelaxScaleY;
			}
			if (_selected) {
				dispatchEvent(new TransformEvent(TransformEvent.UPDATE, [this]));
			}
		...
		public function get hasScaleLimits():Boolean {
			return (appModelinScaleX != -Infinity || appModelinScaleY != -Infinity || appModelaxScaleX != Infinity || appModelaxScaleY != Infinity);
		}
