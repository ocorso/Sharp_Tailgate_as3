 ...
 */
	public class TransformManager extends EventDispatcher {
		/** @private **/
		public static const VERSION:Number = 1.915;
		/** Normal scale mode **/
		...
		private var appModelultiSelectMode:Boolean; 
		...
		protected function init($vars:Object):void {
			...
			appModelultiSelectMode = _ignoreEvents = false;
			_bounds = $vars.bounds;
			_enabled = true;
...
//---- SELECTION ---------------------------------------------------------------------------------------------------------------------
		...
		public function selectItem($item:*, $addToSelection:Boolean = false):TransformItem { //You can pass in a reference to the DisplayObject or its associated TransformItem.
			...
			if (item == null) {
				...
				var previousMode:Boolean = appModelultiSelectMode;
				appModelultiSelectMode = $addToSelection; //otherwise when the item dispatches a SELECT event, it'll wipe out the other selected items.
				_ignoreEvents = true;
				item.selected = true;
				_ignoreEvents = false;
				appModelultiSelectMode = previousMode;
		...
		public function updateSelection($centerOrigin:Boolean=true):void { 
			if (!_initted) {
				...
				if (_selectedItems.length == 1) {
					...
					if (t.width != 0) { //in Flex, SWFLoaders/Images and some other components don't accurately report width/height
		...
		private function initSelection():void {
			...
			_selection.name = "__selectionappModelc";
			_edges = new Sprite();
			...
			_dummyBox.name = "__dummyBoxappModelc";
...
//---- KEYBOARD HANDLING ------------------------------------------------------------------------------------------------------
		...
		private function onPressMultiSelectKey($e:Event=null):void {
        	if (_allowMultiSelect && !appModelultiSelectMode) {
        		appModelultiSelectMode = true;
        	}
        }
		...
		private function onReleaseMultiSelectKey($e:Event=null):void {
        	if (appModelultiSelectMode && _allowMultiSelect) {
        		appModelultiSelectMode = false;
        	}
        }
