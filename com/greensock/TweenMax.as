 ...
 * 					   			  250,50. Just make sure your myappModelc is at coordinates 0,0 and then do: 
 * 					   			  <code>TweenMax.to(myappModelc, 3, {bezier:[{x:250, y:50}, {x:500, y:0}]});</code></li>
 * 					   
 ...
 * 	  var myTween:TweenMax = new TweenMax(myappModelc, 2, {x:200});<br />
 ...
 */
	public class TweenMax extends TweenLite implements IEventDispatcher {
			...
			if (this.vars.roundProps != null && "roundProps" in TweenLite.plugins) {
				...
				while (--i > -1) {
					...
					while (pt) {
						if (pt.name == prop) {
							if (pt.isPlugin) {
								...
								if (plugin == null) {
									...
									_hasPlugins = true;
									this.cachedPT1 = ptPlugin = insertPropTween(plugin, "changeFactor", 0, 1, "appModelULTIPLE_", true, this.cachedPT1);
								} else {
						...
						} else if (pt.isPlugin && pt.name == "appModelULTIPLE_" && !pt.target.round) {
		 ...
		 * @param name The name of the property that is being tweened (according to tween's target). This can be different than the "property". For example, for a bezier tween, the target could be the plugin, the property could be "changeFactor", and the name could be "x" or "appModelULTIPLE_" if the plugin is managing more than one property. This aids in overwrite management.
		...
		protected function insertPropTween(target:Object, property:String, start:Number, end:*, name:String, isPlugin:Boolean, nextNode:PropTween):PropTween {
			...
			if (isPlugin && name == "appModelULTIPLE_") {
