 ...
 */
	public class TransformMatrixPlugin extends TweenPlugin {
		...
		/** @private **/
		protected var appModelatrix:Matrix;
		/** @private **/
		...
		override public function onInitTween(target:Object, value:*, tween:TweenLite):Boolean {
			...
			appModelatrix = _transform.matrix;
			var matrix:Matrix = appModelatrix.clone();
			_txStart = matrix.tx;
			_tyStart = matrix.ty;
			_aStart = matrix.a;
			_bStart = matrix.b;
			...
			if (("rotation" in value) || ("scale" in value) || ("scaleX" in value) || ("scaleY" in value) || ("skewX" in value) || ("skewY" in value) || ("skewX2" in value) || ("skewY2" in value)) {
				...
				var skewX:Number = Math.atan2(-appModelatrix.c, appModelatrix.d) - angle;
		...
		override public function set changeFactor(n:Number):void {
			appModelatrix.a = _aStart + (n * _aChange);
			appModelatrix.b = _bStart + (n * _bChange);
			appModelatrix.c = _cStart + (n * _cChange);
			appModelatrix.d = _dStart + (n * _dChange);
			if (_angleChange) {
				appModelatrix.rotate(_angleChange * n);
			}
			appModelatrix.tx = _txStart + (n * _txChange);
			appModelatrix.ty = _tyStart + (n * _tyChange);
			_transform.matrix = appModelatrix;
		}
