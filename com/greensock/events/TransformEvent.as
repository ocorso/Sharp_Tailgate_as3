 ...
 */
	public class TransformEvent extends Event {
		/** Dispatched constantly while an item is moved by TransformManager (each time the mouse moves). To discern when an interactive move operation is finished (when the user releases the mouse after moving an item), listen for the the <code>FINISH_INTERACTIVEappModelOVE</code> event. **/
		...
		/** Dispatched by TransformItems when they are selected (the <code>SELECT</code> event is <strong>NOT</code> dispatched by a TransformManager instance). **/
